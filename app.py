...
# --- HEADER ---
st.title("Future Wealth Planner 💰"); st.write("A tool to forecast your financial future based on detailed inputs."); st.markdown("---")
...
with st.sidebar:
    ...
    forecast_length = st.number_input("Forecast Length (Years)", 5, 60, 40, 1)
...
with tab_report:
    ...
    if not st.session_state.report_ready:
        ...
        for i, year in enumerate(years[1:]):
            ...
            for col in asset_cols:
                ...
                if age < retirement_age:
                    ...
                    # RMD Logic for Pre-tax accounts
        ...
        for col in drawdown_cols:
            ...
            if first_draw_year:
                age = df.loc[first_draw_year, 'Age']; val = df.loc[first_draw_year, col]
                event_list.append({'Age': age, 'Value': val, 'Event': 'Drawdown Begins', 'Source': col})
        ...
        if event_list:
            events_df = pd.DataFrame(event_list).dropna(); event_markers = alt.Chart(events_df).mark_circle(size=150, stroke='white', strokeWidth=2).encode(x=alt.X('Age:O'), y=alt.Y('Value:Q'), color=alt.Color('Source:N'), tooltip=['Age', 'Event']); final_chart = alt.layer(line_chart, event_markers)
