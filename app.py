...
# This is used to control the visibility of the report tab
if 'report_ready' not in st.session_state:
    st.session_state.report_ready = False

def run_calculation():
    """Sets the flag to show the report tab."""
...
with st.sidebar:
    ...
    st.markdown("---")
    # The calculation is now tied to a function
    st.button("Calculate Financial Future", on_click=run_calculation, use_container_width=True, type="primary")

# --- DEFINE TABS ---
# The report tab is now created conditionally based on session state
...
with tabs[0]: # Income Tab
    ...
    with c1:
        ...
        inc_job1 = st.checkbox("Include in Forecast", True, key="inc_job1")
    ...
    with c2:
        ...
        inc_job2 = st.checkbox("Include in Forecast", True, key="inc_job2")
...
if st.session_state.report_ready:
    with tabs[-1]: # This will be the Forecast tab
        ...
        income_df = pd.DataFrame(index=years, data={'Age': ages})
        event_list = []
        ...
        if inc_job1:
            ...
            if any(v > 0 for v in job1_incomes): event_list.append({'Age': retirement_age, 'Value': job1_incomes[ages.index(retirement_age)-1], 'Event': f'{job1_name} Ends', 'Color': '#d62728'}) # Red
        if inc_job2:
            ...
            if any(v > 0 for v in job2_incomes): event_list.append({'Age': retirement_age, 'Value': job2_incomes[ages.index(retirement_age)-1], 'Event': f'{job2_name} Ends', 'Color': '#d62728'}) # Red
        if inc_ss:
            income_df[ss_name] = [ss_annual_amount if age >= ss_start_age else 0 for age in ages]
            if ss_annual_amount > 0: event_list.append({'Age': ss_start_age, 'Value': 0, 'Event': f'{ss_name} Begins', 'Color': '#2ca02c'}) # Green
        if inc_pension1:
            income_df[pension1_name] = [pension1_annual_amount if age >= pension1_start_age else 0 for age in ages]
            if pension1_annual_amount > 0: event_list.append({'Age': pension1_start_age, 'Value': 0, 'Event': f'{pension1_name} Begins', 'Color': '#2ca02c'}) # Green
        if inc_pension2:
            income_df[pension2_name] = [pension2_annual_amount if age >= pension2_start_age else 0 for age in ages]
            if pension2_annual_amount > 0: event_list.append({'Age': pension2_start_age, 'Value': 0, 'Event': f'{pension2_name} Begins', 'Color': '#2ca02c'}) # Green

        income_columns = [c for c in [job1_name, job2_name, ss_name, pension1_name, pension2_name] if c in income_df.columns]
        income_df['Total Income'] = income_df[income_columns].sum(axis=1) if income_columns else 0
        ...
        if not income_df.empty and 'Total Income' in income_df.columns and not income_df['Total Income'].empty:
            ...
            - Your income is projected to peak in **{peak_income_year}** at age **{income_df.loc[peak_income_year, 'Age']}**, reaching **${peak_income_value:,.0f}**.
            - Over this period, your total estimated income is **${total_lifetime_income:,.0f}**.
            """)
