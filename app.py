...
if st.session_state.report_ready:
    with tabs[-1]:
        ...
        if inc_job1:
            ...
            if retirement_age in ages and any(v > 0 for v in job1_incomes): # BUG FIX
                retirement_index = ages.index(retirement_age)
                if retirement_index > 0:
                    event_list.append({'Age': retirement_age, 'Value': job1_incomes[retirement_index - 1], 'Event': f'{job1_name} Ends', 'Color': '#d62728'})
        if inc_job2:
            ...
            if retirement_age in ages and any(v > 0 for v in job2_incomes): # BUG FIX
                retirement_index = ages.index(retirement_age)
                if retirement_index > 0:
                    event_list.append({'Age': retirement_age, 'Value': job2_incomes[retirement_index - 1], 'Event': f'{job2_name} Ends', 'Color': '#d62728'})
        if inc_ss:
        ...
        event_markers = alt.Chart(events_df).mark_circle(size=150, stroke='white', strokeWidth=2).encode(
            ...
            color=alt.Color('Color:N', scale=None), tooltip=['Age', 'Event']
        )
        ...
        if not income_df.empty and 'Total Income' in income_df.columns and not income_df['Total Income'].empty:
            ...
            if peak_income_value > 0:
                st.write(f"- Your income is projected to peak in **{peak_income_year}** at age **{income_df.loc[peak_income_year, 'Age']}**, reaching **${peak_income_value:,.0f}**.")
